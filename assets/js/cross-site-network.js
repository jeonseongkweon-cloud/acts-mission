...
(function(){
  ...
  function getActsClient(){
    ...
    }catch(error){console.error('ACTS Supabase client error:',error);return null;}
  }
  ...
  async function enforceActsAdmin(){
    ...
    try{
      ...
      if(!client){console.error('ACTS admin guard: Supabase config not found');return;}
      const {data:{session},error:sessionError}=await client.auth.getSession();
      if(sessionError||!session){if(!/\/admin\.html$/i.test(path)) location.replace('/admin.html');return;}
      const {data:isAdmin,error:adminError}=await client.rpc('acts_is_admin');
      if(adminError||isAdmin!==true){
        await client.auth.signOut();
        if(/\/admin\.html$/i.test(path)){
          const msg=document.getElementById('message');if(msg) msg.textContent='ACTS 관리자 권한이 없는 계정입니다.';
        }else location.replace('/admin.html?denied=1');
      }
    }catch(error){console.error('ACTS admin guard error:',error);if(!/\/admin\.html$/i.test(path)) location.replace('/admin.html');}
  }

  function addGrandOpenStyles(){
    if(document.getElementById('acts-grand-open-style')) return;
    const style=document.createElement('style');style.id='acts-grand-open-style';
    style.textContent=`
      ...
      .acts-community-compose h3{margin:0 0 7px;color:#061A33;font-size:19px}.acts-community-compose p{margin:0 0 14px;color:#667085;font-size:13px;line-height:1.6}
  ...
  function setupAdminReportCard(){
    if(!isAdminDashboard||document.getElementById('actsReportAdminCard')) return;
  ...
  async function setupCommunityWriter(){
    ...
    const client=getActsClient();if(!client) return;addGrandOpenStyles();
    const box=document.createElement('section');box.className='acts-community-compose';box.id='actsCommunityCompose';toolbar.parentNode.insertBefore(box,toolbar);
    const {data:{session}}=await client.auth.getSession();
    if(!session){box.innerHTML='<h3>🤝 ACTS 가족 이야기 나누기</h3><p>회원소통 글쓰기는 로그인한 ACTS 회원이 이용할 수 있습니다.</p><a class="acts-login-cta" href="/member-center/login.html?redirect=%2Fcommunity-board.html">회원 로그인 후 글쓰기</a>';return;}
    const meta=session.user.user_metadata||{};const defaultAuthor=meta.full_name||meta.name||meta.display_name||(session.user.email||'ACTS 회원').split('@')[0];
    box.innerHTML='<h3>🤝 ACTS 가족 이야기 나누기</h3><p>인사, 협력 요청, 지역 소식, 함께 나누고 싶은 이야기를 올려주세요. 개인정보와 광고성 내용은 올리지 말아주세요.</p><input id="actsCommunityTitle" maxlength="120" placeholder="제목을 입력하세요"><textarea id="actsCommunityContent" maxlength="5000" placeholder="내용을 입력하세요"></textarea><div class="acts-compose-actions"><button type="button" class="acts-compose-btn" id="actsCommunitySubmit">글 등록</button><span class="acts-compose-status" id="actsCommunityStatus"></span></div>';
    const submit=box.querySelector('#actsCommunitySubmit'),status=box.querySelector('#actsCommunityStatus');
    submit.addEventListener('click',async()=>{
      const title=box.querySelector('#actsCommunityTitle').value.trim(),content=box.querySelector('#actsCommunityContent').value.trim();
      if(title.length<2||content.length<5){status.textContent='제목과 내용을 조금 더 입력해 주세요.';return;}
      submit.disabled=true;status.textContent='등록 중입니다...';
      try{
        const {error}=await client.from('posts').insert({category:'community',title,content,author:String(defaultAuthor).slice(0,80),user_id:session.user.id,is_published:true,moderation_status:'visible'});if(error) throw error;
        box.querySelector('#actsCommunityTitle').value='';box.querySelector('#actsCommunityContent').value='';status.textContent='등록되었습니다.';
        if(typeof window.loadPosts==='function') await window.loadPosts();else setTimeout(()=>location.reload(),500);
      }catch(error){console.error('ACTS community insert error:',error);status.textContent='등록하지 못했습니다. 로그인 상태를 확인해 주세요.';}finally{submit.disabled=false;}
    });
  }
  ...
  async function submitPostReport(postId){
    const client=getActsClient();if(!client) return;
    const {data:{session}}=await client.auth.getSession();
    if(!session){if(confirm('게시글 신고는 로그인한 회원만 가능합니다. 로그인 페이지로 이동할까요?')){const returnTo=encodeURIComponent(location.pathname+location.search);location.href='/member-center/login.html?redirect='+returnTo;}return;}
    const reason=prompt('신고 사유를 입력해주세요.\n예: 광고·스팸 / 개인정보 노출 / 부적절한 내용 / 기타');if(!reason) return;
    const clean=reason.trim().slice(0,80);if(clean.length<2) return;
    try{
      const {error}=await client.from('acts_post_reports').insert({post_id:Number(postId),reporter_user_id:session.user.id,reason:clean});
      if(error){if(error.code==='23505'){alert('이미 신고한 게시글입니다.');return;}throw error;}alert('신고가 접수되었습니다. ACTS 운영진이 확인하겠습니다.');
    }catch(error){console.error('ACTS post report error:',error);alert('신고를 접수하지 못했습니다. 잠시 후 다시 시도해주세요.');}
  }

  function setupPostReport(){
    if(!isPostDetail) return;addGrandOpenStyles();
    const postId=new URLSearchParams(location.search).get('id');if(!postId||!/^[0-9]+$/.test(postId)) return;
    const root=document.getElementById('detailRoot');if(!root) return;
    const inject=()=>{const card=root.querySelector('.detail-card');if(!card||card.querySelector('.acts-report-wrap')) return false;const wrap=document.createElement('div');wrap.className='acts-report-wrap';wrap.innerHTML='<button type="button" class="acts-report-btn">⚑ 신고하기</button>';wrap.querySelector('button').addEventListener('click',()=>submitPostReport(postId));card.appendChild(wrap);return true;};
  ...
  const dock=document.createElement('div');dock.id='acts-external-sites-dock';dock.innerHTML='<button class="ae-toggle" type="button">관련 전문기관 ↗</button><div class="ae-panel"><div class="ae-title">EXTERNAL PARTNER SITES</div><a href="https://ipma1822-png.github.io/taekwonkumdo/" target="_blank" rel="noopener">태권검도 WTKF ↗</a><a href="https://ipma.kr/" target="_blank" rel="noopener">국제경찰무도연합회 IPMA ↗</a><a href="https://idp.ai.kr/" target="_blank" rel="noopener">국제드론순찰대 IDP ↗</a><small>ACTS와 무도·드론 사이트는 운영 영역을 분리하며, 외부 사이트는 새 창에서 열립니다.</small></div>';document.body.appendChild(dock);dock.querySelector('.ae-toggle').onclick=()=>dock.classList.toggle('open');document.addEventListener('click',e=>{if(!dock.contains(e.target))dock.classList.remove('open')});
})();
